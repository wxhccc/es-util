...
function createConfig(output, plugins = [], input, tsOptions) {
  ...
  return {
    ...
    output: Array.isArray(output)
      ? output.map((cfg) => Object.assign(cfg, { banner }))
      : {
          ...output,
          banner
        },
...
function getConfig(env) {
  ...
  if (env === 'development')
    return [esConfig, createConfig(babelOutput, [babelPlugin])]
  const umdOut = {
    file: pkg.unpkg,
    name: 'EsUtil',
    format: 'umd',
  ...
  const singleFiles = {
    ...
    'value-string-switch': path.resolve(
      __dirname,
    ...
    'event-target-emitter': path.resolve(
      __dirname,
      'src/event-target-emitter.ts'
    ),
  ...
  const singleFileCjsCfg = createConfig(
    {
      format: 'cjs',
      dir: path.resolve(__dirname, distDir),
  ...
  return [esConfig, createConfig(babelOutput, [babelPlugin]), singleFileCjsCfg]
}
