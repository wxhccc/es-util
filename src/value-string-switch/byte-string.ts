...
const configCreater = (standard: string, unit: string) => {
  ...
  const standBase: { [key:string]: number } = { metric: 1000, iec: 1024, jedec: 1024 };
  return {
...
export function byteStringify (byteNum: number| string, options?: Options) {
  ...
  const getValue = (value: number) => Number.isInteger(value) ? value : value.toFixed(precision);
  let resUnit = '';
...
export function byteParse (byteStr: string, options = {}) {
