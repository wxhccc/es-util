...
/* 身份证合法性验证 */
export function ChinaIdCardValid(idNo: string) {
  ...
  return (
    idCardReg.test(idNumber) &&
    provCodes.includes(idNumber.substr(0, 2)) &&
    lastCodeCheck()
  )
}
// 公式验证函数
export function formulaValidate(string: string, variables?: string[]) {
  ...
  return (
    ...
    !blns.test(string) &&
    bmatch() &&
    (!isArr(variables) || varInList(variables))
  )
}
